...
export default function Restaurant ({ restaurant, extras }: { restaurant: BusinessType, extras?: ReviewResponseType & { distance?: any, duration?: any } }) {
    return (
                        ...
                        <div className="sm:self-end flex items-start justify-between sm:flex-col sm:items-end mt-4">
                            ...
                            <ul className="mt-2 flex flex-wrap">
                                { restaurant.categories.length > 0 && restaurant.categories.map((category: CategoryType, key: number) => (
                                    <li key={key} className="font-barlow mr-1 mb-1 w-fit p-1 px-2 rounded-2xl text-sm capitalize border-2 border-stone-800">{category.title}</li>
                                ))}
                            </ul>
                        </div>
                    </div>
