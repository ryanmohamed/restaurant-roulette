...
export default function Restaurant ({ restaurant, extras }: { restaurant: BusinessType, extras?: ReviewResponseType }) {
    return (
        <div className="w-full relative">
            ...
            <p className="font-poppins mt-2 text-right">{restaurant?.location?.address1}, {restaurant.location?.city}, {restaurant.location?.zip_code}</p>

            <div className="mt-6 flex flex-col md:flex-row">
                <div className="self-center mb-4 md:mb-0 w-fit md:max-w-[40%] h-fit rounded-2xl shadow-black shadow-md overflow-hidden md:mr-4">
                    <img 
                        src={restaurant?.image_url || "http://placeholder.co/500/500"} 
                        alt="restaurant image" //todo: add restaurant specifc alt text
                        className="h-[300px] w-auto object-cover saturate-150 brightness-90" 
                    />
                </div>
                <div className="flex flex-col flex-grow">
                    <a href={restaurant?.url} rel="noreferrer" target="_blank" className="btn centered text-center text-stone-100 mb-4 h-10 self-center w-[300px] md:w-full">Order Now</a>
